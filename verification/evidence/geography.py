...
class GeographyEvidenceCollector:
    ...
    def collect(
        ...
        candidate_lat: float,
        candidate_lon: float,
        reverse_geocode_data: dict | None = None,
        ...
        if reverse_geocode_data:
            ...
            evidence.nearby_landmarks = self._check_nearby_landmarks(
                reverse_geocode_data
            )
    ...
    def _check_reverse_consistency(
        ...
        return bool(
            ref_country
            and rev_country
            and (ref_country in rev_country or rev_country in ref_country)
        )
    ...
    def _check_admin_hierarchy(
        ...
        if (
            ref_country
            and rev_country
            and (ref_country not in rev_country and rev_country not in ref_country)
        ):
            return False
        ...
        if (
            ref_state
            and rev_state
            and (ref_state not in rev_state and rev_state not in ref_state)
        ):
            # Allow fuzzy match
            from rapidfuzz import fuzz
    ...
    def _check_nearby_educational(self, reverse_data: dict) -> bool:
        ...
        return tags.get("amenity") in ("school", "university", "college")
    ...
    def _check_nearby_roads(self, reverse_data: dict) -> bool:
        ...
        return bool(reverse_data.get("road") or reverse_data.get("street"))
